<html>
 <body>
<head>
<h1> Turn Old Laptop Display Into External Monitor </h1>
</head>
<body bgcolor= "LightCyan">

<h2> Materials needed:</h2><br>
<ul>
  <li> Old unused laptop </li>
  <li> Controller board that works with display </li>
  <li> Video cable </li>
  <li> Frame (Optional) </li>
</ul> <br>

<h2> Making The External Display </h2>
<ol>
  <li> Step 1: Disassemble the display from the laptop. </li>
  <center><img src = "/images/lcd2.jpg" alt ="LCD2"></center> <br>
  <li> Step 2: Once the display is disassembled look for the serial number in the back of the display.</li>
  <li> You will need this information in order to get the correct controller board. </li>
  <center><img src = "/images/lcd1.jpg" alt ="LCD1"></center> <br>
  <li> Step 3: Once you have the controller board, simply plug it in to the display</li>
  <center><img src = "/images/lcd3.jpg" alt ="LCD3"></center> <br>
  <li> Your new display is ready to go!</li>
  <center><img src = "/images/lcd4.jpg" alt ="LCD4"></center> <br>
  <li>Optional: You can choose to build a frame for the display and/or conroller board.</li>
  <li>Depending on the model of your display you may find 3D printing files online that will do the trick.</li>

<p><a href="index.md"> Home Page </a></p>

</body>

</html>
